...
class HomeBaseController extends BaseController
{
    ...
    protected function fetch($template = '', $vars = [], $replace = [], $config = [])
    {
        ...
        // $where["status"] = ["eq","1"];
        // $where["lx"] = ["eq","1"];
        // $gjwy     = Db::name("link")->where($where)->select();
        // $where["lx"] = ["eq","2"];
        // $sszzq     = Db::name("link")->where($where)->select();
        // $where["lx"] = ["eq","3"];
        // $zzqgj     = Db::name("link")->where($where)->select();
        // $where["lx"] = ["eq","4"];
        // $msmh     = Db::name("link")->where($where)->select();
        // $where["lx"] = ["eq","5"];
        // $xwzhl     = Db::name("link")->where($where)->select();
        // $where["lx"] = ["eq","6"];
        // $yqlj     = Db::name("link")->where($where)->select();
        $data_area_zx_all            = Db::name("area")->where("parent_id=1")->order("convert(name using gbk)")->select();
        $data_area_sm_all            = Db::name("area")->where("path like '0-2%'")->order("convert(name using gbk)")->select();
        $this->assign('theme_vars', $more['vars']);
        $this->assign('theme_widgets', $more['widgets']);
        // $this->assign('gjwy', $gjwy);
        // $this->assign('sszzq', $sszzq);
        // $this->assign('zzqgj', $zzqgj);
        // $this->assign('msmh', $msmh);
        // $this->assign('xwzhl', $xwzhl);
        // $this->assign('yqlj', $yqlj);
        $this->assign("data_area_zx_all",$data_area_zx_all);
        $this->assign("data_area_sm_all",$data_area_sm_all);
