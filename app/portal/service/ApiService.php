...
class ApiService
{
    ...
    public static function articles($param)
    {
        ...
        $areaid       = empty($param['areaid']) ? 0 : $param['areaid'];
        $dwxz       = empty($param['dwxz']) ? 0 : $param['dwxz'];
        ...
        if (!empty($categoryIds)) {
            ...
            $field = !empty($param['field']) ? $param['field'] : 'post.id,post.post_title,post.create_time,min(category_post.category_id) as category_id';
            ...
            if($areaid>0){
                array_push($join, ['__CATEGORY_DWXZ__ c', 'category_post.category_id=c.category_id']);
