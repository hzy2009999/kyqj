...
class AdminArticleController extends AdminBaseController
{
    ...
    public function edit()
    {
        ...
        $portalCategoryModel = new PortalCategoryModel();
        $categoryTree        = $portalCategoryModel->adminCategoryTree($postCategoryIds);
        $this->assign('category_tree', $categoryTree);
    ...
    public function delete()
    {
        ...
        if (isset($param['id'])) {
            ...
            if ($resultPortal) {
                ...
                Db::name('portal_tag_post')->where(['post_id'=>$id])->delete();
                Db::name('post_hits')->where(['post_id'=>$id])->delete();
            }
            $this->success("删除成功！", '');
        ...
        if (isset($param['ids'])) {
            ...
            if ($result) {
                ...
                Db::name('portal_tag_post')->where(['post_id' => ['in', $ids]])->delete();
                Db::name('post_hits')->where(['post_id'=>['in', $ids]])->delete();
    ...
    public function publish()
    {
        ...
        if (isset($param['ids'])) {
            $ids = $this->request->param('ids/a');
            $area = Db::name("area")->alias("a")->join("portal_post b","a.id=b.area_id","LEFT")->where("b.id=".$ids[0])->field("a.*")->find();
            if (substr_count($area["path"],'-')<=2) {
                // $this->success("非村庄地区文章不需要审核！", cmf_url('AdminArticle/index',array('page'=>$pagenum)));
                $this->success("非村庄地区文章不需要审核！",'');
            }
            if (isset($param["yes"])) {

                $portalPostModel->where(['id' => ['in', $ids]])->update(['post_status' => 1, 'published_time' => time()]);

                // $this->success("审核成功！", cmf_url('AdminArticle/index',array('page'=>$pagenum)));
                $this->success("审核成功！",'');
            }
            ...
            if (isset($param["no"])) {
                ...
                // $this->success("取消审核成功！", cmf_url('AdminArticle/index',array('page'=>$pagenum)));
                $this->success("取消审核成功！",'');
            }
        }else{
            // $this->error("请选择需要审核的文章！", cmf_url('AdminArticle/index',array('page'=>$pagenum)));
            $this->error("请选择需要审核的文章！",'');
        }
